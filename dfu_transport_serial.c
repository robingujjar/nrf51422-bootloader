...
void my_simple_uart_put(uint8_t cr)
{
...
void my_simple_uart_putstring(const uint8_t *str)
{
  uint_fast8_t i = 0;
  uint8_t ch = str[i++];
  while (ch != '\0')
  {
    my_simple_uart_put(ch);
    ch = str[i++];
  }
}
...
uint32_t dfu_transport_update_start(void)
{
    ...
    APP_ERROR_CHECK(err_code);
	   ...
	   //printf("\n \r Wellocme to new Uart");
    //my_simple_uart_putstring((const uint8_t *)" \n\r JAI MATA DI ==> : ");
