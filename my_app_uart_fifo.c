...
extern void nrf51422_erase_app(uint8_t no_of_pages);
...
extern void write_firmware_byte(uint32_t data);

void flash_write_packet(uint8_t  * p_event_message_buffer,uint8_t len){
	 uint32_t data =0;
	 uint8_t i =0;
	
	 for(i=0;i<len;i+=4){
	  data = data| (p_event_message_buffer[i+0]<<0);
	  data = data| (p_event_message_buffer[i+1]<<8);
	  data = data| (p_event_message_buffer[i+2]<<16);
	  data = data| (p_event_message_buffer[i+3]<<24);
		write_firmware_byte(data);
		data =0;
	 }
...
void firmware_line(uint8_t  * p_event_message_buffer,uint8_t  mesg,  uint8_t len ){
	  ...
	  data = data| (p_event_message_buffer[0]<<0);
	  data = data| (p_event_message_buffer[1]<<8);
	  data = data| (p_event_message_buffer[2]<<16);
	  data = data| (p_event_message_buffer[3]<<24);
	  //ram_address+=4;
  	//printf("write_firmware_byte %08x\r\n",data);
	  write_firmware_byte(data);
	  //*ram_address = data;
    //ram_address++;	
	...
	/* for(l=0;l<len;l+=4){
	  data = data| (p_event_message_buffer[l+0]<<0);
	  data = data| (p_event_message_buffer[l+1]<<8);
	  data = data| (p_event_message_buffer[l+2]<<16);
	  data = data| (p_event_message_buffer[l+3]<<24);
		write_firmware_byte(data);
		data =0;
	 }*/
        ...
        case ASSIGN_CHANNEL:
            ...
            break;
				case OPEN_CHANNEL:					
					  ...
					  //printf("\n OPEN_CHANNEL ");
            uartreader_send_response((uartreader_response_t)0x89);
            ...
            break;
             ...
             //write_line(evt);				 
			      ...
			      break;			 
			 case MESG_ERASE_APP:
				    no_of_pages = *(&evt->data[3]);
            //nrf51422_erase_app(no_of_pages);
			      ANT_RequestMessage(*(&evt->data[3]),MESG_ERASE_APP);
            
            break;
        case MESG_RESET_AND_RUN:
             //uartreader_send_response((uartreader_response_t)0x93);
				     ANT_RequestMessage(*(&evt->data[3]),MESG_RESET_AND_RUN);
...
void command_handler_from_stm32xx(uint8_t ch){
			...
			if(m_buffer_index==0){ //start of the message 
              ...
              crc ^= m_evt.data[m_buffer_index];
							...
							if( m_buffer_index >= UARTREADER_MAX_LEN-1  //this is for a max value case 
							            || m_buffer_index >= this_msg_len+3 ){   //this is what is working now.
										if(crc !=0){				
											printf("address %08x crc error %02x\r\n",ram_address,crc);
											printf("m_evt.data[0] %02x\r\n",m_evt.data[0]);
											printf("m_evt.data[1] %02x\r\n",m_evt.data[1]);
											printf("m_evt.data[2] %02x\r\n",m_evt.data[2]);
											printf("m_evt.data[3] %02x\r\n",m_evt.data[3]);
											printf("m_evt.data[4] %02x\r\n",m_evt.data[4]);
											printf("m_evt.data[5] %02x\r\n",m_evt.data[5]);
											printf("m_evt.data[6] %02x\r\n",m_evt.data[6]);
											printf("m_evt.data[7] %02x\r\n",m_evt.data[7]);
										}			 
